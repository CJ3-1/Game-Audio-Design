...
namespace DigitalRuby.RainMaker
{
    public class BaseRainScript : MonoBehaviour
    {
        ...
        [Header("定时雨水切换设置")]
        [Tooltip("是否开启自动定时切换雨水强度（0→1循环）")]
        public bool AutoToggleRain = true;
        [Tooltip("每种强度持续时长（秒），0持续10秒，1持续10秒")]
        ...
        private bool isRainMax = false;

        public GameObject jiming;
        ...
        private void AutoSwitchRainIntensity()
        {
            if (!AutoToggleRain) return;
            ...
            if (rainTimer >= rainDuration)
            {
                rainTimer = 0;
                isRainMax = !isRainMax;
                RainIntensity = isRainMax ? 1f : 0f;

                // 逻辑：0=激活jiming，1=关闭jiming
                if (RainIntensity == 0f)
                {
                    jiming.SetActive(true);
                }
                else
                {
                    jiming.SetActive(false);
                }
            }
        }
